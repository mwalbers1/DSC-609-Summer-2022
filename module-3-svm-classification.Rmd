...
```{r}
dd$rideable_type <- factor(dd$rideable_type, levels = c("classic_bike", "docked_bike", "electric_bike"))
```
...
```{r}
dummy <- dummyVars("~ rideable_type", data = dd, levelsOnly = TRUE)
```
...
```{r}
dummy.df <- data.frame(predict(dummy, newdata = dd))
```
...
dd$start_hour <- format(as.POSIXct(dd$started_at), format = "%H")
```

```{r Ending Hour of Bike trip}
dd$end_hour <- format(as.POSIXct(dd$ended_at), format = "%H")
```
...
Plot features from both classes, casual and member
